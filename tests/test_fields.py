...
class TestFields(BaseTest, unittest.TestCase):
    ...
    def _fixture_template(self, data_ok, expected=None, data_fail=None, msg=None):
        ...
        response = self.client.post(
            "/fieldsdoc/", data=json.dumps(data_fail), content_type="application/json"
        )
    ...
    def test_url_field(self):
        self._fixture_template(
            data_ok={"a": "http://google.com"},
            data_fail={"a": "foobar"},
            msg={
                "a": "ValidationError (FieldsDoc:None) (Invalid"
    ...
    def test_email_field(self):
        self._fixture_template(
            data_ok={"b": "heller.stanislav@gmail.com"},
            data_fail={"b": "invalid@email"},
            msg={
                "b": "ValidationError (FieldsDoc:None) (Invalid email address: invalid@email (domain validation failed): ['b'])"
            },
        )

    def test_uuid_field(self):
        self._fixture_template(
            data_ok={"g": "ddbec64f-3178-43ed-aee3-1455968f24ab"},
            data_fail={"g": "foo-bar-baz"},
            msg={
                "g": "ValidationError (FieldsDoc:None) (Could "
    ...
    def test_map_field(self):
        self._fixture_template(
            data_ok={"f": {"x": "foo", "y": "bar"}},
            data_fail={"f": {"x": 1}},
            msg={
                "f": "ValidationError (FieldsDoc:None) "
    ...
    def test_embedded_in_list(self):
        ...
        try:
            ...
            self.assertListEqual(
                json_data["o"], [{"a": "foo", "b": 789}, {"a": "baz", "b": 456}]
            )
        finally:
            d.delete()
    ...
    def test_db_field_name(self):
        ...
        response = self.client.post(
            "/fieldsdoc/", data='{"n": "hello"}', content_type="application/json"
        )
        json_data = response.get_json()
